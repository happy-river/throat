"""Peewee migrations -- 026_message_types_and_mailboxes.py
...
def migrate(migrator, database, fake=False, **kwargs):
    ...
    if not fake:
        ...
        for msg in Message.select():
            ...
            UserMessageMailbox.create(mid=msg.mid, uid=msg.sentby, mailbox=MessageMailbox.SENT)
...
def rollback(migrator, database, fake=False, **kwargs):
    ...
    if not fake:
        ...
        query = (Message.select().
                 join(UserMessageMailbox, pw.JOIN.INNER,
                      on=((UserMessageMailbox.mid == Message.mid) &
        ...
        for msg in (Message.select()
                    .join(UserIgnores, pw.JOIN.RIGHT_OUTER,
                          on=((UserIgnores.uid == Message.receivedby) &
